...
if train_model == True:
    ...
    # tensorboard 
    if train_model == True:
        writer = SummaryWriter(f'resnet50-{EPOCHS}')
    ...
    torch.save(model, f'./models/resnet50-{EPOCHS}.pth')
    ...
    MODEL_NAME = f"resnet50-{EPOCHS}" 
    device = torch.device("cuda" if torch.cuda.is_available else "cpu")
    criterion = nn.CrossEntropyLoss()
    model = torch.load(f'./models/resnet50-{EPOCHS}.pth')
