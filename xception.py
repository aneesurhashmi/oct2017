...
if train_model == True:
    ...
    # model = xception(num_classes=1000, pretrained='imagenet').to(device)
    model = timm.create_model('xception', pretrained=True, num_classes=4)
    # model = models.xception(weights="xception_Weights.DEFAULT").to(device)
    ...
    device = torch.device("cuda" if torch.cuda.is_available else "cpu")
    criterion = nn.CrossEntropyLoss()
    model = torch.load(f'./models/xception-{EPOCHS}.pth')
    model = model.to(device)
